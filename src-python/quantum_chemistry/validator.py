...
class MaterialValidationResult:
    ...
    def compute_summary(self) -> None:
        ...
        self.overall_passed = self.pass_rate >= 0.80
...
def validate_ysz(
    ...
    props.append(_check("lattice_a", sim.get("lattice_a_angstrom"), ref.get("lattice_a_angstrom"), tol=0.05, unit="\u00c5", failure_reason="Basis set (LANL2DZ) or active space truncation."))
    props.append(_check("lattice_c", sim.get("lattice_c_angstrom"), ref.get("lattice_c_angstrom"), tol=0.05, unit="\u00c5", failure_reason="Basis set or active space truncation."))
    props.append(_check("oxygen_vacancy_formation_energy", sim.get("oxygen_vacancy_formation_ev"), ref.get("oxygen_vacancy_formation_ev"), tol=0.3, unit="eV", failure_reason="Active space (6e/6o) excludes deep O 2s contributions."))
    props.append(_check("ionic_conductivity_proxy", sim.get("ionic_conductivity_proxy"), ref.get("ionic_conductivity_proxy"), tol=0.2, unit="(relative)", failure_reason="Cluster model neglects long-range cooperative diffusion."))
    props.append(_check("dielectric_constant", sim.get("dielectric_constant"), ref.get("dielectric_constant"), tol=5.0, unit="(dimensionless)", failure_reason="Finite cluster + active space truncation underestimates polarisability."))
    props.append(_check("ground_state_energy_kcal_mol", sim.get("ground_state_energy_kcal_mol"), ref.get("ground_state_energy_kcal_mol"), tol=2.0, unit="kcal/mol", failure_reason="VQE energy above chemical accuracy threshold."))
    mvr = MaterialValidationResult(material="Yttria-Stabilised Zirconia (YSZ)", canon_ref="C65", properties=props)
    mvr.compute_summary()
    logger.info("YSZ validation: %d/%d passed (%.0f%%)", mvr.n_passed, mvr.n_tested, mvr.pass_rate * 100)
    return mvr
...
def validate_bts(
    ...
    ref = load_reference_data(reference_path)
    props: List[PropertyResult] = []
    props.append(_check("lattice_a", sim.get("lattice_a_angstrom"), ref.get("lattice_a_angstrom"), tol=0.05, unit="\u00c5", failure_reason="ECP (LANL2DZ/Ba) introduces basis incompleteness error."))
    props.append(_check("lattice_c", sim.get("lattice_c_angstrom"), ref.get("lattice_c_angstrom"), tol=0.05, unit="\u00c5", failure_reason="Ti off-centering displaced geometry approximation."))
    props.append(_check("curie_temperature_k", sim.get("curie_temperature_k"), ref.get("curie_temperature_k"), tol=30.0, unit="K", failure_reason="Vegard-law Tc interpolation is sublinear for BTS."))
    props.append(_check("spontaneous_polarisation_c_m2", sim.get("spontaneous_polarisation_c_m2"), ref.get("spontaneous_polarisation_c_m2"), tol=0.05, unit="C/m\u00b2", failure_reason="Dipole-moment proxy neglects periodic boundary corrections."))
    sim_pe = (sim.get("piezoelectric_tensor") or {}).get("e33_c_m2")
    ref_pe = ref.get("e33_c_m2")
    props.append(_check("piezoelectric_e33", sim_pe, ref_pe, tol=1.5, unit="C/m\u00b2", failure_reason="Finite-field perturbation underestimates e33 for 5-atom cluster."))
    mvr = MaterialValidationResult(material="Ba(Ti,Sn)O3 (BTS)", canon_ref="C66", properties=props)
    mvr.compute_summary()
    logger.info("BTS validation: %d/%d passed (%.0f%%)", mvr.n_passed, mvr.n_tested, mvr.pass_rate * 100)
    return mvr
...
def validate_alscn_gan(
    ...
    ref = load_reference_data(reference_path)
    props: List[PropertyResult] = []
    props.append(_check("alscn_lattice_a", sim.get("alscn_a_angstrom"), ref.get("alscn_a_angstrom"), tol=0.05, unit="\u00c5", failure_reason="Literature geometry used directly; no geometry optimisation."))
    props.append(_check("gan_lattice_a", sim.get("gan_a_angstrom"), ref.get("gan_a_angstrom"), tol=0.05, unit="\u00c5", failure_reason="Literature geometry used directly; no geometry optimisation."))
    sim_ba = sim.get("band_alignment") or {}
    props.append(_check("delta_ec_band_offset", sim_ba.get("delta_ec_ev"), ref.get("delta_ec_ev"), tol=0.15, unit="eV", failure_reason="Cluster ionisation proxy; full periodic slab deferred (Canon C67 \u00a74.4)."))
    sim_pd = sim.get("polarisation_discontinuity") or {}
    props.append(_check("delta_p_total", sim_pd.get("delta_p_total_c_m2"), ref.get("delta_p_total_c_m2"), tol=0.02, unit="C/m\u00b2", failure_reason="SP from Fichtner 2019; PE from strain estimate, not self-consistent."))
    props.append(_check("sigma_2deg", sim.get("sigma_2deg_cm2"), ref.get("sigma_2deg_cm2"), tol=ref.get("sigma_2deg_tolerance_cm2", 1e13), unit="cm\u207b\u00b2", failure_reason="Sheet charge model; Poisson-Schr\u00f6dinger solver deferred."))
    mvr = MaterialValidationResult(material="Al0.7Sc0.3N/GaN Interface (AlScN:GaN)", canon_ref="C67", properties=props)
    mvr.compute_summary()
    logger.info("AlScN/GaN validation: %d/%d passed (%.0f%%)", mvr.n_passed, mvr.n_tested, mvr.pass_rate * 100)
    return mvr
...
def write_validation_report(
    ...
    # Pre-extract status string — backslash escapes inside f-strings
    # are illegal on Python 3.11 (only supported from 3.12+)
    overall_status = "\u2705 PASS" if report.all_passed else "\u26a0\ufe0f PARTIAL / \u274c FAIL"
    lines = [
    ...
    for mvr in report.materials:
        mat_status = "\u2705 PASS" if mvr.overall_passed else "\u26a0\ufe0f NEEDS REVIEW"
        lines += [
            f"## {mvr.material} ({mvr.canon_ref})",
            "",
            f"**Pass rate:** {mvr.n_passed}/{mvr.n_tested} testable properties "
            f"({mvr.pass_rate:.0%}) \u2014 {mat_status}",
    ...
    lines += [
        ...
        "",
        "Properties marked \u274c are documented here with likely causes and recommended remediation paths.",
        "",
        "### Common Causes",
        "",
